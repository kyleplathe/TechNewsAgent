...
- **Hand-pick from CI (no browser): `LIST_CANDIDATES` + `PICK_URLS`.** The local web picker can't run in GitHub Actions, so CI hand-pick is a **two-phase, URL-keyed** flow (indices change between runs as feeds refresh, so picks are matched on **URL**, not number):
